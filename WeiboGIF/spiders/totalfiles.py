...
# @Last Modified time: 2016-06-16 23:34:30
...
from WeiboGIF.items import WeiboGIFItem
from WeiboGIF.startuser import StartUrls
...
class WeiboGIFSpider(CrawlSpider):
    ...
    rules = (
        # 搜索界面翻页链接 #
        Rule(LinkExtractor(allow=(r'search/user/?keyword=.*?&page=\d+',)), follow = True),
        # 搜索界面中用户链接 #
        Rule(LinkExtractor(allow=(r'.*?f=search_\d+',)), follow = True,
            callback = 'parse_User'),
        # # 用户界面中翻页链接 #
        # Rule(LxmlLinkExtractor(allow = (r'.*?page=\d+'), restrict_xpaths=("//div[@id='pagelist']/form/div/a[1]")), follow = True, callback = 'parse_User'),
    )
